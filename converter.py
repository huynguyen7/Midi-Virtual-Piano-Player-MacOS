...
def main():
    import sys
    if len(sys.argv) == 2:
        midi_file = "./mid-src/" + sys.argv[1] + ".mid"
        if not os.path.exists(midi_file):
            print(f"Error: file not found '{midi_file}'")
            return 1
    else:
        print('Needs an input file as argument.')
        return 1
    ...
    song_file_path = "./sheets/%s_song.txt" % sys.argv[1]
    sheet_file_path = "./sheets/%s_sheet_conversion.txt" % sys.argv[1]
    
    midi.save_song(song_file_path)
    midi.save_sheet(sheet_file_path)
